...
# -------------------------
# Load ViT-L/16 model (224x224 resolution)
# -------------------------
def prepare_model(model_name="google/vit-large-patch16-224"):
...
def run_one_image(img: Image.Image, processor, model, device):
    # Processor resizes to 224x224 automatically
    inputs = processor(images=img, return_tensors="pt").to(device)
    
    with torch.no_grad():
        outputs = model(**inputs)  # forward pass
        # Option A: CLS token (most common)
        latent = outputs.last_hidden_state[:, 0, :].squeeze().cpu().numpy()
        # Option B: pooled output (sometimes closer to classification head input)
        # latent = outputs.pooler_output.squeeze().cpu().numpy()
...
processor, model = prepare_model("google/vit-large-patch16-224") # 1x1024 features
model.to(device)

data_path = "/mnt/d/Naved/Data/IDRiD/idrid516_orig/"
save_path = "/mnt/d/Naved/Outputs/idrid516_orig/features/idrid516_orig_vitlarge224_features.csv"
